                ...
                RunCommand(args =>
                {
                    ...
                    if (args.Length < 2)
                        throw new ConsoleCommandException(
                            "First argument must be a valid player name, who you want to teleport. Also he needs to be online rightnow.");
                    var playerName = args[1].Replace('_', ' ');
                    var player = Player.s_players.Find(x => x.GetPlayerName() == playerName);
                    if (!player) throw new ConsoleCommandException($"Target player with name {playerName} not found");
                    var tombstone = ZDOMan.instance.GetImportantZDOs(hash).ToList().First(x =>
